package day12_switch_statement;

import java.util.Scanner;

/*
Ask the user how many days they will stay at the hotel
Then ask the user how many people are in their party (how many people are staying in the room)

Use those two information to determine the price and room type for the hotel based on the below data:

	party size: 1
	room type: single room
	price: number of days * 100

	party size: 2
	room type: double room
	price: number of days * 125

	party size: 3 or 4
	room type: large room
	price: number of days * number of people * 150

	party size: 5, 6, or 7
	room type: suite
	price: number of days * 5000

	any other party size:
		Display one message:
			Sorry we don't have any available rooms for that size party

 */
public class StayAtHotel {
    public static void main(String[] args) {
        Scanner input = new Scanner (System.in);
        System.out.println("How many night you are going to stay");
        int nights = input.nextInt();
        System.out.println("How many people will be in the party");
        int partySize = input.nextInt();
        String roomType = "";
        int price =0;

        switch (partySize) {
            case 1:
                roomType = "single room";
                price = nights * 100;
                break;
            case 2:
                roomType = "double room";
                price = nights * 125;
                break;
            case 3:
            case 4:
                roomType = "large room";
                price = nights * partySize * 150;
                break;
            case 5:
            case 6:
            case 7:
                roomType = "suite";
                price = nights * 5000;
                break;
            default:
                System.out.println("Sorry we don't have any available rooms for that size party");
                break;



        }
        System.out.println("For " + partySize + " we have available " + roomType + " and the price will be $" + price);




    }

}
