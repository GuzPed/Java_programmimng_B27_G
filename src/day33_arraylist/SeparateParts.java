...
list3: {1, 2, 3, 4, 5, 6}
    ...
    public static void main(String[] args) {
        ...
        letters.removeIf(p-> !Character.isLetter(p.charAt(0)));// p isevery element, every element each character of the String, but those characters were String type, so in order to get a cgar type we did charAt(0). Character methods needed char type
        System.out.println(letters);

        ArrayList<String> numbers = new ArrayList<>(Arrays.asList(s.split("")));
        numbers.removeIf( p-> !Character.isDigit(p.charAt(0)));
